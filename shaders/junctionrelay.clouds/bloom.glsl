...
//
// Star-pattern bloom on sunlit cloud edges from pass 0 (cloud simulation).
// 4 directions (H, V, 2 diagonals) x 6 taps = 24 reads + 1 center + 1 original = 26.
// Warm tint controlled by bloomWarmth — golden light through clouds.
// Additive composite at bloomIntensity strength.
// Alpha pass-through: preserves cloud density state from pass 0 for feedback.
//
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Pass through alpha from pass 0 (cloud density state for temporal feedback).
